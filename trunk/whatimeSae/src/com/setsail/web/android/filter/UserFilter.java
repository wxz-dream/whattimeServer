...
public class UserFilter implements Filter{
	...
	public void doFilter(ServletRequest request, ServletResponse response,
		...
		if(!contains(noLoginAuthURL, url)){
			String RMIME = SstringUtils.getNotNull(request.getParameter(MIME)).isEmpty()
					? SstringUtils.getNotNull(request.getParameter(MIME.toLowerCase()))
					: SstringUtils.getNotNull(request.getParameter(MIME));
			String RUUID = SstringUtils.getNotNull(request.getParameter(UUID)).isEmpty()
					? SstringUtils.getNotNull(request.getParameter(UUID.toLowerCase()))
					: SstringUtils.getNotNull(request.getParameter(UUID));
					
			if(userRepository == null){
				ApplicationContext ac = WebApplicationContextUtils.
						getWebApplicationContext(((FilterConfig) request).getServletContext());
			...
			if(user == null || !user.getMime().equals(RMIME)){
				response.getWriter().write(
					JSON.toJSONString(new SystemState(StateEnum.STATE_NOLOGIN))
				);
				return;
			}
		}
		...
		if(classNames != null){
			...
			for(String className : classNames){
				...
				if(controller != null){
					...
					Controller c = AnnotationUtils.findAnnotation(controller, Controller.class);
					
					if(c != null){
						//controller类RequestMapping注解
						RequestMapping crm = AnnotationUtils.findAnnotation(controller, RequestMapping.class);
						//url前缀
						String[] prefixs = crm  == null ? new String[0] : crm.value();
						
						Method[] ms = controller.getMethods();
						if(ms != null){
							for(Method m : ms){
								Auth auth = AnnotationUtils.findAnnotation(m, Auth.class);
